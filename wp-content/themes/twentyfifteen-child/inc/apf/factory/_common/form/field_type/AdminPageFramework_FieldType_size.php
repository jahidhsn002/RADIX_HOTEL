...
class hub_AdminPageFramework_FieldType_select extends hub_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".localhost.com-field-select .localhost.com-input-label-container {vertical-align: top; }.localhost.com-field-select .localhost.com-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        $_oSelectInput = new hub_AdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div class='localhost.com-input-label-container localhost.com-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class hub_AdminPageFramework_FieldType_size extends hub_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".localhost.com-field-size input {text-align: right;}.localhost.com-field-size select.size-field-select {vertical-align: 0px; }.localhost.com-field-size label {width: auto; } .form-table td fieldset .localhost.com-field-size label {display: inline;}";
    }
    protected function getField($aField) {
    ...
    protected function _getFieldOutputByLabel($isKey, $sLabel, array $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $isKey, $aField['label']), "<div class='localhost.com-input-label-container localhost.com-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>", $this->_getNumberInputPart($aField, $_aBaseAttributes, $isKey, is_array($aField['label'])), $this->_getUnitSelectInput($aField, $_aBaseAttributes, $isKey, is_array($aField['label'])), "</div>", $this->getElementByLabel($aField['after_label'], $isKey, $aField['label']));
        return implode('', $_aOutput);
    }
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span class='localhost.com-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new hub_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
