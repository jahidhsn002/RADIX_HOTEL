...
class hub_AdminPageFramework_Form_View___Script_CollapsibleSection extends hub_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializehub_AdminPageFrameworkCollapsibleSections();
    });              
    ...
    jQuery( document ).bind( 'admin_page_framework_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializehub_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializehub_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.localhost.com-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.localhost.com-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.localhost.com-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.localhost.com-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.localhost.com-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.localhost.com-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.localhost.com-collapsible-sections-title, .localhost.com-collapsible-section-title' ).enablehub_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.localhost.com-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'localhost.com-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.localhost.com-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.localhost.com-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.localhost.com-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.localhost.com-sectionset' ).siblings().andSelf().find( '> .localhost.com-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.localhost.com-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .localhost.com-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.localhost.com-sections' ).children( '.localhost.com-section' ).children( '.localhost.com-section-table' ).children( 'caption' ).children( '.localhost.com-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enablehub_AdminPageFrameworkCollapsibleButton = function() {
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'localhost.com-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.localhost.com-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .localhost.com-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.localhost.com-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='localhost.com-collapsible-toggle-all-button-container'>" . "<span class='localhost.com-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
