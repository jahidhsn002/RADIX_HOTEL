...
abstract class hub_AdminPageFramework_Form_View___Fieldset_Base extends hub_AdminPageFramework_FrameworkUtility {
    public $aField = array();
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + hub_AdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedSScripts_Widget) {
            new hub_AdminPageFramework_Form_View___Script_Widget;
        ...
        new hub_AdminPageFramework_Form_View___Script_Utility;
        new hub_AdminPageFramework_Form_View___Script_OptionStorage;
        new hub_AdminPageFramework_Form_View___Script_AttributeUpdator;
        new hub_AdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new hub_AdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
        ...
        if ( ! jQuery( '#{$sFieldsContainerID} .localhost.com-repeatable-field-buttons' ).length ) { 
            // Adds the buttons
            jQuery( '#{$sFieldsContainerID} .localhost.com-field' ).prepend( $_sButtonsHTML ); 
        }
    }     
    jQuery( '#{$sFieldsContainerID}' ).updatehub_AdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enablehub_AdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='localhost.com-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
class hub_AdminPageFramework_Form_View___Fieldset extends hub_AdminPageFramework_Form_View___Fieldset_Base {
    public function get() {
        $_aOutputs = array();
        $_oFieldError = new hub_AdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aField['_section_path_array'], $this->aField['_field_path_array'], $this->aField['error_message']);
        $_aOutputs[] = $_oFieldError->get();
        $_oFieldsFormatter = new hub_AdminPageFramework_Form_Model___Format_Fields($this->aField, $this->aOptions);
    ...
    private function _getEachFieldOutput(array $aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
        ...
        $_oSubFieldFormatter = new hub_AdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
        $aField = $_oSubFieldFormatter->get();
        $_oFieldAttribute = new hub_AdminPageFramework_Form_View___Attribute_Field($aField);
    ...
    private function _getFinalOutput(array $aFieldset, array $aFieldsOutput, $iFieldsCount) {
        $_oFieldsetAttributes = new hub_AdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
    ...
    private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
        ...
        $_oFieldsAttributes = new hub_AdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
    ...
    private function _getExtras($aField, $iFieldsCount) {
        $_aOutput = array();
        $_oFieldDescription = new hub_AdminPageFramework_Form_View___Description($aField['description'], 'localhost.com-fields-description');
